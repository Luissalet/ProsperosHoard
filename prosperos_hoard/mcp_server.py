...
def voice_audiobook(
    ...
    preset: Optional[str] = None, language: Optional[str] = None, lexicon: Optional[dict[str, str]] = None,
) -> dict[str, Any]:
    ...
    body = {"text": text, "source_path": source_path, "title": title, "format": format, "project": project,
            ...
            "voice": {"engine_id": engine_id, "voice_id": voice_id, "voice_ref": voice_ref, "speed": speed,
                      "preset": preset, "language": language, "lexicon": lexicon}}
...
def studio_recipe_export(production: str, name: Optional[str] = None, overwrite: bool = False) -> dict[str, Any]:
    ...
    Exporting the same production again refreshes its recipe; a recipe of that name made from another
    production is never replaced silently: the call fails with recipe_exists - pick another name, or
    overwrite=true (only when the user wants that recipe replaced).
